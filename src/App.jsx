      ...
      {showInfo && (
        <div className="modal-overlay" onClick={() => setShowInfo(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <button className="modal-close" onClick={() => setShowInfo(false)} aria-label="Close">
              ✕
            </button>
            <h2>How Dialectica works</h2>
            <p>
              Two AI debaters — each with its own persona and voice — take turns arguing a topic you
              choose. Everything runs <strong>100% in your browser</strong>: no backend, no API keys,
              no server costs. The LLM and the voices never leave your device.
            </p>
            <ul>
              <li>
                <strong>LLM:</strong> A small instruction-tuned language model (default Llama 3.2 3B)
                runs on-device via <em>WebGPU</em> using WebLLM. One engine serves both debaters; the
                system prompt is swapped each turn to switch personas. The model is downloaded once
                and cached in your browser (IndexedDB) for instant subsequent loads.
              </li>
              <li>
                <strong>Voice:</strong> Streaming text-to-speech turns each debater's text into speech
                as it's generated. The default <em>Web Speech</em> engine uses your browser's built-in
                voices — zero download, instant. An optional <em>Piper</em> neural engine (~40 MB per
                voice) gives higher-quality speech and runs on the CPU so the GPU stays with the LLM.
              </li>
              <li>
                <strong>Pipeline:</strong> While one debater is speaking, the next debater's turn is
                already being generated and synthesized in the background, keeping the back-and-forth
                snappy with minimal gaps.
              </li>
            </ul>
            <h3>Disclaimer</h3>
            <p className="disclaimer">
              The arguments are generated by a small language model running locally and are for
              entertainment and demonstration purposes only. They may be <strong>historically,
              logically, or scientifically inaccurate</strong>, and may contain <strong>hallucinated
              facts</strong> (confident but false claims). <strong>Offensive language</strong> may also
              occur despite the safety guardrails built into the model. Do not rely on these arguments
              for factual claims, and verify anything you hear against trusted sources. The views
